...
export default function Carousel({itemContainer,items,CarouselComponent}) {
    ...
    return (
        ...
        <button className="scroll-buttons"
    id="slideBack"
    type="button"
    onClick={(e) => sideScroll("left",itemContainer)}>&#60;</button>
        <div ref={itemContainer} className='featured-items'>

            {
                items.map(card=>(
                    <CarouselComponent card={card}/>))
    ...
    id="slideBack"
    type="button"
    onClick={(e) => sideScroll("right",itemContainer)}>&#62;</button>
    </div>
    )
}
